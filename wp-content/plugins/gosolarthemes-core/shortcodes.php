...
function zozo_soundcloud_shortcode( $atts ) {
	...
	$atts = extract(shortcode_atts(
		...
		return '<div class="soundcloud-shortcode"><iframe width="'.$width.'" height="'.$height.'" src="https://w.soundcloud.com/player/?url=' . urlencode($url) . '&amp;show_comments=' . $comments . '&amp;auto_play=' . $autoplay . '&amp;color=' . $color . '&amp;buying=' . $buy_like . '&amp;liking=' . $buy_like . '&amp;show_artwork=' . $show_artwork . '"></iframe></div>';
...
function zozo_day_counter_shortcode( $atts, $content = null ) {
	...
	if( ( isset( $button_text ) && $button_text == '' ) ) {
		...
		$button_text = esc_html__('Get a discount', 'gosolar');
	...
	wp_enqueue_script( 'gosolar-zozo-countdown-plugin-js' );

	wp_enqueue_script( 'gosolar-zozo-countdown-js' );
...
function zozo_fullwidth_box_shortcode( $atts, $content = null ) {
	...
	if( isset( $overlay ) && $overlay == 'yes' ) {
		...
		if( isset( $overlay_color ) && $overlay_color != '' ) {
			...
			$bg_color_rgb = gosolar_hex2rgb( $overlay_color );
...
function zozo_sitemap_shortcode() {
	...
	$output .= '<div class="sitemap-wrapper">';
		...
		$output .= '<div class="sitemap-pages">';
			...
			$output .= '<h6 class="parallax-title text-left">'. esc_html__('Pages', 'gosolar') .'</h4>';
		...
		$output .= '<div class="sitemap-categories">';
			...
			$output .= '<h6 class="parallax-title text-left">'. esc_html__('Categories', 'gosolar') .'</h4>';
		...
		$output .= '<div class="sitemap-archives">';	
			...
			$output .= '<h6 class="parallax-title text-left">'. esc_html__('Archives', 'gosolar') .'</h4>';
