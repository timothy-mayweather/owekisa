...
class GoSolarItemRatings {
	...
	protected $plugin_path						= GOSOLAR_THEME_DIR;

	protected $plugin_url						= GOSOLAR_THEME_URL;
	...
	*/
	...
	function __call( $method, $args ) {
		...
		//Detect calls to callback method
	...
	*/
	...
	public function enqueue_admin_scripts() {
		...
		$js_inc_path 	= GOSOLAR_THEME_URL . '/js/';

		$css_inc_path 	= GOSOLAR_THEME_URL . '/css/';

		

		//Enqueue scripts for Rate It js plugin

		wp_enqueue_script( 'gosolar-zozo-rate-it', $js_inc_path . 'rate-it/gosolar-jquery.rateit.min.js', array('jquery'), '1.0.22' );

		

		//Enqueue this plugin's script

		wp_enqueue_script( 'gosolar-zozo-item-ratings', $js_inc_path . 'rate-it/gosolar-zozo-ratings-script.js', array('gosolar-zozo-rate-it'), '1.0' );

		

		//Enqueue stylesheet for rate it plugin - can be de-enqueued and replaced by themes :)

		wp_enqueue_style( 'gosolar-zozo-item-ratings-stars', $css_inc_path . 'gosolar-rateit-admin.css', array(), '1.0' );
	...
	*/
	...
	private function localize_script() {
		...
		wp_localize_script( 'gosolar-zozo-item-ratings', $object, $js_vars );
	...
	*/
	...
	public function add_custom_column_content( $config_option_key, $column_name, $post_ID ) {
		...
		if ($column_name == $column_slug) {  
	        ...
	        echo apply_filters( 'gosolar_item_ratings_column_stars', $output, $column_name, $option, $post_ID );
	...
	* Loops all plugin config options and Sets params and calls
	...
	* Callback function for called in $this->init_custom_meta_boxes
	...
	public function add_custom_meta_box( $config_option_key ) {
		...
		foreach( self::$class_config as $option ) {
			...
			if( $option['meta_key'] === $config_option_key ) {
		        ...
		        echo apply_filters( 'gosolar_item_ratings_metabox_stars', $output, $option, $post_ID );
